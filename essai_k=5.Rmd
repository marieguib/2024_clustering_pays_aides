...
```{r}
table(gpe.kmeans)
grp <- which.min(table(gpe.kmeans))
```
Ce deuxième cluster contient 47 pays.\
Nous identifions le pays le plus pauvre :

```{r}
df_donnees_sc <- as.data.frame(donnees.sc)
P1 <- rownames(df_donnees_sc)[df_donnees_sc$gpe.kmeans==grp]
P1
```
...
```
Le graphique ci-dessus n'est pas très concluant quant à l'hypothèse de partition en deux groupes. Nous nous posons la question d'une éventuelle partition en 3 ou 6 groupes de pays en observant les sauts d'inertie intra-classes.
...
```
Ce tracé de la perte d'inertie intra-classes nous indique 3 ou 6 groupes.
...
Grâce à cette méthode, nous confirmons notre choix de partition en 3 groupes.
...
donnees.sc_defav  <- cbind(donnees.sc_defav,gpe.ward2)
donnees.sc_defav$gpe.ward2 <- as.factor(donnees.sc_defav$gpe.ward2)
# plot(gpe.ward2)
interp_cah2 <- catdes(donnees.sc_defav,num.var = 10)
interp_cah2
```
