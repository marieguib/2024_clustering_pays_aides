...
Nous pouvons remarquer que les données sont dans des unités différentes et les ordres de grandeur sont très variables. Ce boxplot nous montre une différence notable entre les variables. Il nous incite donc à standardiser les données.

```{r}
boxplot(donnees)
```

Nous standardisons les données :
...
On peut en déduire qu'on a un groupe nécessiteux (groupe 2) et l'autre plus aisé (groupe 1).
...
En conclusion, nous allons de nouveau réaliser une CAH mais cette fois-ci sur les données provenant du groupe "défavorisé", c'est-à-dire le groupe 2, pour réduire le nombre d'individus au sein de chaque cluster.\
L'objectif est de répartir au mieux notre budget.

## Deuxième partitionnement

Pour cette deuxième CAH, nous sélectionnons parmis nos données de base, les pays appartenant au groupe nécessiteux, c'est à dire au groupe 1 (obtenu à la suite du cutree)
...
```{r}
D <- dist(donnees_defav,method="euclidean")^2
```
